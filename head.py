...
class OBB(Detect):
    """YOLO OBB detection head for detection with rotation models (Physics-Informed)."""
    ...
    def __init__(self, nc: int = 80, ne: int = 1, ch: tuple = ()):
        ...
        # 🔥 创新注入：单根导线光学透射率 (Transmittance) 预测头
        c_t = max(ch[0] // 4, 1)
        self.cv_t = nn.ModuleList(nn.Sequential(Conv(x, c_t, 3), Conv(c_t, c_t, 3), nn.Conv2d(c_t, 1, 1)) for x in ch)

    def forward(self, x: list[torch.Tensor]) -> torch.Tensor | tuple:
        """Concatenate and return predicted bounding boxes, class probabilities, and transmittance."""
        ...
        # 2. 提取物理透射率 t
        t = torch.cat([self.cv_t[i](x[i]).view(bs, 1, -1) for i in range(self.nl)], 2)
        t = t.sigmoid().clamp(min=1e-2, max=0.99)

        if not self.training:
            self.angle = angle
            self.t = t  # 静态保留物理场，供后续自定义部署或可视化脚本调用

        # 3. 调用父类 Detect 获取基础特征
        x = Detect.forward(self, x)
        ...
        # ====================================================================
        # 🔥 核心修正：解耦验证流与特征流
        # - 推理张量 (x[0]): 剥离 t，仅返回 [x[0], angle]，完美兼容 NMS 和 ProbIoU 算子。
        # - 原始特征 (x[1]): 保留 t，组成 (x[1], angle, t)，确保 Validator 在计算验证集 Loss 时不报错。
        # ====================================================================
        return torch.cat([x, angle], 1) if self.export else (torch.cat([x[0], angle], 1), (x[1], angle, t))
